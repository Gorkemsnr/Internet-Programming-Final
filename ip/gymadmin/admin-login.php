...
if (isset($_POST["submit"])){
  ...
  if(!$username){
    ...
    $kullanici_sor =$db->prepare('SELECT * FROM users WHERE username=? AND password=?');
    $kullanici_sor->execute([$username,$password]);
     $say=$kullanici_sor->rowCount();
    if($say==1){
      $_SESSION['username']=$username;
      echo "başarıyla giriş yaptınız,yönlendiriliyorsunuz";
      header('location: /ip/gymadmin/billing.php');
			...
			<div class="left">
				...
				<div class="contact">
					<form action="" method="post">
						...
						<button class="submit" name ="submit" >Hadi Gidelim</button>
					</form>
				</div>
			</div>
			<div class="right">
				<div class="right-text">
					...
					<h5>Spor salonu Yönetim Sistemi</h5>
				</div>
